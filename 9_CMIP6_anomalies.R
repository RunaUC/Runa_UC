...
# Anomalies (subtract mean from observations)  --------
  ...
  do_CMIP_anomalies <- funciton(obs) {
    ...
    cdo_code <- paste0("cdo ") # the format to subtract the mean from the observations is: cdo[fill in the stuff here] sub obs mn optput_file
      system(cdo_code)
    }
